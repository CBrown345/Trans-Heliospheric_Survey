...
matplotlib.rcParams['text.latex.preamble'] = r'\usepackage{starfont} \usepackage{amsmath}'
...
def mod_broken2( x, c, xa, xb, a1, a2, a3, force=0 ):
	...
	return y
def pws(v: float, r: float, t: float, a:float) -> float:
    G = 6.67e-11
    m_h = 1.67e-27 #[kg]
    m_sun = 1.99e30 #[kg]
    k = 1.38e-23 #[J/K]
    lamb = (G * m_h * m_sun) / (2 * a * k * t)
    psi = (((1/2) * m_h * (v**2)) / (k * t))
    return (-3 - 4 * log(lamb / 2) + 4 * log(r/a) + ((2 * lamb) / (r/a)) - psi + log(psi))
def pws_explicit(r,t,a):
	return least_squares(pws, x0 = 5e7, args = (r,t,a), bounds= (0, inf)).x 
def pws_explicit_p(r, t, r_0, a):
    if r < r_0:
        return pws_explicit(r, t, a)
    else:
        return pws_explicit(r_0, t, a)
v_pws_explicit = vectorize(pws_explicit)
v_pws_explicit_p =vectorize(pws_explicit_p)
def B(r, B_0, a):
    return B_0 * power( r, -1) * power(a +  power( r, -2), .5) 
#-------------------------------------------------------------------------------
...
def plot_msc( key, scaled = True, special = False ) :
	...
	inch_fig_y = ( inch_mrgn_bottom + 2. * inch_plot_y
	                                +      inch_hist_y + inch_mrgn_top   )
	...
	if ( key == 'b' ) :
		...
		yscale = 'log'
		ylim_scaled = [ 0.003, 170. ]
		ylim_unscaled = [ 0.008, 900. ]
		unit = 1.
	elif ( key == 'np' ) :
		dkey = 'np'
		...
		corner_fit_var = False
		yscale = 'log'
		ylim_scaled = [ 0.00008, 300. ]
		ylim_unscaled = [ 0.0005, 1500. ]
		unit = 1.
	elif ( key == 'vp' ) :
		dkey = 'vp_m'
	...
	fig_key.set_xlim( [ 0., 5. ] ) ### Changed from 4. for Solar Orbiter
	...
	sc_name = [ r'$\rm PSP$'         , r'$\rm Solar~Orbiter$', r'$\rm Helios~1$'  , 
            	r'$\rm Helios~2$'    , r'$\rm Ulysses$'     , r'$\rm Mariner~2$' , 
             	r'$\rm Mariner~10$'  , r'$\rm Cassini$'     , r'$\rm Pioneer~10$', 
              	r'$\rm Pioneer~11$'  , r'$\rm New~Horizons$', r'$\rm Voyager~1$' , 
             	r'$\rm Voyager~2$'   ]
	...
	p1 = 'New_Fits/Data_v2025.03/individual_spc/binned_unscaled/'
	p2 = '_coho1hr_merged_mag_plasma_'
	p3a = '_v2025.03.hf'
	p3b = '_v2025.03.hf'

	sc_fl = [ p1 + 'psp'          + p2 + '20180101_20231001' + p3b,
	...
	p = 'New_Fits/Data_v2025.03/all_data/'

	if ( scaled ) :
		dat = h5py.File( p + 'all_spacecraft_data_85_binned_scaled_v2025.03.hf', 'r' )
	else :
		dat = h5py.File( p + 'all_spacecraft_data_85_binned_v2025.03.hf', 'r' )
	...
	#Filtering for NaNs and np.infs
	tk_pre = where( ( isfinite( dat_r   ) ) & ( isfinite( dat_10  ) ) &
	                ( isfinite( dat_25  ) ) &
	                ( isfinite( dat_50  ) ) & ( isfinite( dat_75  ) ) &
	                ( isfinite( dat_90  ) ) & ( isfinite( dat_num ) )   ) [0]
	...
	tk_sel = where( ( pre_num >= 25 ) & ( pre_r <  80. ) )[0]   # Selected (for fits)
	tk_tms = where( ( pre_num >= 25 ) & ( pre_r >= 80. ) )[0]   # After termination shock
	...
	for i in range( n_tms ) :   # For each bin after the termination shock . . .
		...
		fig_comp.plot( tms_r[i], tms_50[i],
		               color='w', linestyle='', marker='o',
		               markersize=3.0, markeredgewidth=1.0,
	...
	if ( key in [ 'loss' ] and (not special)) :   # Parameters fit with no breaks
	...
	elif ( key in [  'np', 'vp', 'vpr', 'va', 'na' ] and (not special)) :   # Parameters fit with one break
	...
	elif ( key in [ 'b','tp', 'betap' ] and (not special)) :      # Parameters fit with two breaks
		...
		if ( key in [ 'va' ] ) :
			...
			bounds = ( ( 0  , 0.2,  1., -inf, -inf, -inf ),
			           ( inf, 1. , 10.,  inf,  inf,  inf )  )
		elif ( key in [ 'tp' ] ) :
			p0 = [ 1., 0.2, 5., 0.0, -0.7, -0.2 ]
		elif ( key in [ 'betap' ] ) :
			p0 = [ 1., 0.3, 30., 1.0, -0.5, 1.0 ]
		else :
			p0 = [ 1., 0.3, 4., -2., -1., -0.5 ]
		...
		#print( '     @ r = 40 au  ==> ' + str( val_40 ) )
		#print( '     ratio (40:1) ==> ' + str( val_40 / val_01 ) )
	if(special and key == "vp"):
		sel_r_SI = 1.496e11 * sel_r #switch to meters for curve fit
		sel_50_SI = 1000. * 412. * sel_50
		popt,pcov = curve_fit( v_pws_explicit_p, sel_r_SI, sel_50_SI,
		                 p0 = (1.e6, 1.e11,1.e13), bounds = (0,inf)  )
		print(popt)
		arr_x = geomspace(1e10, 1.0e13, 1000)
		arr_x_scaled = []
		arr_v_scaled = []
		arr_v_scaled_piecewise= []
		for x in arr_x:
			arr_v_scaled.append(v_pws_explicit(x,popt[0], popt[2]) / (412* 1000))
			arr_v_scaled_piecewise.append(v_pws_explicit_p(x, *popt) / (412 * 1000))
			arr_x_scaled.append(x / 1.496e11)
   
		#Performing curve fit and creating arrays to plot fit
		fig_comp.vlines((popt[1] / 1.496e11), 0, 2, color = "c", linewidth = 1,linestyles = "dashed")
		fig_comp.plot(arr_x_scaled, arr_v_scaled_piecewise, color = "b")
		fig_comp.plot(arr_x_scaled, arr_v_scaled, color = "b", linestyle = "dashed", linewidth = 1)
		#styling
		#fig_comp.legend([r'$r_0$ at ' + str(popt[1] / 1.496e11)[0:4] + " au"], loc = "lower right")
	if (special and key == "b"):
		popt, pcov = curve_fit(B, sel_r, sel_50, sigma = sel_50)
		popt_std = sqrt(diag(pcov))
		bm_curves = []
		print(popt[0]*5.5e-9, sqrt(popt[1]))
		print(popt_std[0]*5.5e-9, popt_std[1])
		# for i in range(10000):
		# 	dummy = []
		# 	rand_popt0 = random.normal(popt[0], 2 * popt_std[0])
		# 	rand_popt1 = random.normal(popt[1], 2 * popt_std[1])
		# 	for r in sel_r:
		# 		dummy.append(B(r, rand_popt0, rand_popt1))
		# 	bm_curves.append(dummy)
		# bm_10 = percentile(bm_curves, 10, axis = 0)
		# bm_90 = percentile(bm_curves, 90, axis = 0)	
		# fig_comp.fill_between(sel_r, bm_10, bm_90, color = '#C4C4C4')
		fig_comp.plot(sel_r, B(sel_r, *popt), color = "b")
		custom_lines = [Line2D([0], [0], color='none', lw=0) for _ in range(4)]
		fig_comp.legend(custom_lines, [r'$B_r (r_1 ,\theta, \phi - r\Omega/v) = $' + str(popt[0] * 5.5e-9)[0:4] + str(popt[0] * 5.5e-9)[-4:] + ' T',
             r'$cos(\theta)r_e\Omega/v = $' + str(sqrt(popt[1]))[0:4]
             ], loc = "upper right")

	# Output the figure.

	if ( scaled ) :
		fname = 'Figures/updated_parkerfit_' + key + '.pdf'
...
for key in arr_key :
	...
	for scaled in [ True ] :  # , False
		plot_msc( key, scaled=scaled, special = True )