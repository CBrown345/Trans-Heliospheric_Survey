...
def plot_msc( key, scaled=True ) :
	...
	if ( scaled ) :
		dat = pd.read_hdf( 'Data_Processed/msc/smin_msc_scaled_v2.hf' ) 
        
	else :
		dat = pd.read_hdf( 'Data_Processed/msc/smin_msc_unscaled_v2.hf' )  
	dat_r   = array( dat['sc_r_iqr_50']  )
	...
	elif ( key in [  'np', 'vp', 'vpr', 'va', 'tp' ] ) :   # Parameters fit with one break
	...
	if ( scaled ) :
		fname = 'Figures/Solar Min Power Law 7-3-25/plot_scaled_' + key + '.pdf'
		#fname = 'eps/plot_scaled_' + key + '.eps'
	else :
		fname = 'Figures/Solar Min Power Law 7-3-25/plot_unscaled_' + key + '.pdf'
