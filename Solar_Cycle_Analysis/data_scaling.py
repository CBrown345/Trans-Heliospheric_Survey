...
for sc in scs:
    ...
    print(fname)
    df = pd.read_hdf(fname[0])
    df_scaled = scale_data(df=df, key_list=n_key_list)
    df_ind.append(df_scaled)
    df_scaled.to_hdf(f"Data_Processed/Individual/{sc}/{sc}_scaled_unbinned", key="df", mode="w")
df_all = pd.concat(df_ind)
bin_data(df_all, "Data_Processed/msc/all_spacecraft_binned_scaled", n_bin=n_bin, key_list = n_key_list)
