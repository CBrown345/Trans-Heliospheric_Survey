...
for sc in scs:
    ...
    fnames = np.sort(glob(data_dir + f"*/*.cdf", recursive = True)) #recursively grab all files for sc
    #print(fnames)
    ...
    for fname in fnames:
        ...
        if (sc == "psp" or sc == "solo"):
        ...
        elif (sc == "pioneer10" or sc == "pioneer11" or sc == "helios1" or sc == "helios2"):
            d["Epoch"] = dat["Epoch"][:]
            d["bm"] = dat["B"][:]
            d["np"] = dat["protonDensity"][:]
            d["vp_m"] = dat["flowSpeed"][:]
            d["Tp"] = dat["protonTemp"][:]
            ...
            d["bm"] = dat["ABS_B"][:]
            d["np"] = dat["protonDensity"][:]
            d["vp_m"] = dat["plasmaFlowSpeed"][:]
            d["Tp_S"] = dat["protonTempSmall"][:]
            d["Tp_L"] = dat["protonTempLarge"][:]
            d["Tp"] = np.mean([d["Tp_S"], d["Tp_L"]], axis=0)
            d["sc_r"] = dat["heliocentricDistance"][:]
            d['heliographicLatitude'] = dat["heliographicLatitude"][:]
            d['heliographicLongitude'] = dat["heliographicLongitude"][:]
        elif(sc == "voyager1" or sc == "voyager2"):
            d["Epoch"] = dat["Epoch"][:]
            d["bm"] = dat["ABS_B"][:]
            d["np"] = dat["protonDensity"][:]
            d["vp_m"] = dat["V"][:]
